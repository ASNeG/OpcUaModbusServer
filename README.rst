...
OpcUaModbusServer is an OPC UA application based on `ASNeG OPC UA Stack`_.
...
Before install OpcUaModbusServer you must install `ASNeG OPC UA Stack`_. 
...
OpcUaModbusServer uses `Sphinx`_ to generate its documentation and it is ready for hosting on `RTD`_.
